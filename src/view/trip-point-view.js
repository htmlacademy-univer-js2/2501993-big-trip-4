...
import { humanizeDate, getDuration, getDate, getTime } from '../utils/trip-point-date.js';

const createOfferList = (allOffers, checkedOffers) => {
  if (!allOffers) {
    return '';
  }
  let offerItems = '';
  allOffers.offers.forEach((offer) => {
    if (checkedOffers.includes(offer.id)) {
      offerItems += `<li class="event__offer">
...
const createTripPointTemplate = (tripPoint, destinations, allOffers) => {
  ...
  const tripTypeOffers = allOffers.find((offer) => offer.type === type);
  ...
  return `<li class="trip-events__item">
    <div class="event">
      ...
      <ul class="event__selected-offers">
        ${createOfferList(tripTypeOffers, offers)}
      </ul>
